...
M4.4D define submissão de decisão por trecho no core Rust. A validação recalcula o pacote a partir dos artefatos atuais, confere IDs e hashes, exige cobertura de todos os trechos e vincula evidência a cada referência citada quando o veredito é `supported`. Evidência sem texto ou bloqueada não sustenta esse veredito. O resultado inclui hash da submissão e `attestationStatus: unverified`; é apenas validação estrutural, sem autenticar revisor, persistir decisões, alterar o QA para `pass` ou liberar TTS. Uma etapa posterior precisa definir atestação confiável, revisão de claims e tratamento de fontes que continuam sem texto.
