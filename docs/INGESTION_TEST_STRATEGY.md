...
## M4.5I — comparação canônica para revisão

Rust/WASM agora compara tokens técnicos do texto nativo e do candidato OCR com relatório vinculado ao recibo. O caso `PR0CEDURE`/`PROCEDURE` aparece como dois tokens divergentes, incluindo um token com dígito. Quando entradas ou saída excedem limites, o relatório é explicitamente truncado ou rejeitado; a contagem se chama `differingTokenLowerBound`. Nenhuma contagem de tokens aprova fidelidade: o estado permanece `review_required`, inclusive quando a contagem é zero. Ver ADR 0021.
...
## M4.5C — integração real com corpus local

O teste opt-in `apps/web/src/adapters/local_corpus.test.ts` aceita `AUDIOBOOK_CORPUS_CICS_PDF` e/ou `AUDIOBOOK_CORPUS_APOSTILA_PDF`. Sem essas variáveis, os casos ficam ignorados no CI; com elas, verificam hash, PDF.js `extractPdf`, migração/construção pelo WASM real e estados de qualidade/eligibilidade. O teste não imprime intencionalmente texto ou caminhos dos PDFs; uma falha de leitura do sistema ainda pode incluir o caminho local no erro, portanto executar apenas em ambiente privado. Foi executado localmente com ambos os arquivos: 2 casos passaram. O manual gerou 105 páginas `corrupted`, com unidades sinalizadas `blocked`; a apostila gerou 75 páginas `good`, sem a flag de glifo privado. Isso valida o pipeline em Node/Vitest, não o Worker dentro do navegador nem a qualidade de OCR.

Páginas 1 e 27 do manual foram renderizadas e inspecionadas. Pequenos tokens visuais e a busca exata na extração PDF.js foram registrados em `work/goldens/cobol-cics-v1.local.json`, ignorado pelo Git. É uma referência candidata, não um golden aprovado: na página 27 há sobreposição de linhas perto do rodapé, e a ausência de dois substrings exatos na camada nativa pode decorrer de espaçamento, glifos privados ou ambos. Próxima medição deve usar coordenadas/recortes e revisão humana antes de calcular taxa de recuperação por OCR.
