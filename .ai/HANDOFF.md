...
- O bundle gerado e o script de regeneração estão versionados. Testes de integração carregam o `.wasm` real, inclusive o caso de página sem texto. Web STANDARD passou com 78/78 testes, typecheck e build. Rust fmt/test/clippy passaram localmente usando toolchain GNU de 32 bits. Commit `ce48c3a` publicado na branch remota; os dois primeiros jobs Rust remotos passaram, mas Web falhou no gate de comparação byte a byte entre hosts. O gate foi ajustado e aguarda novo resultado.
