...
- O usuário pediu uma entrega ampla e um loop local de aprendizado para ambiguidades. O batch implementa três partes operacionais: extração Rust de regras a partir de correções humanas explícitas, banco IndexedDB local de registros derivados e agregação determinística que retorna sugestões ao revisor. Não há modelo neural, serviço externo, SQLite, retreinamento de pesos ou correção automática.
