...
- Gates após o ajuste final: Rust fmt, 44 testes e Clippy passaram; WASM build, typecheck, 140 testes Web e build passaram. Smoke Chromium repetido com o WASM final; `git diff --check` passou. Revisão independente final aprovou sem achados P0–P2. Publicação e CI remotos ainda dependem de autenticação GitHub disponível nesta sessão.
