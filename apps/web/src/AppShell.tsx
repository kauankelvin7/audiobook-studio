...
function ProjectHeader({ fileName, pageCount, saved, audioBusy, chapterCount }: { fileName: string; pageCount: number; saved: boolean; audioBusy: boolean; chapterCount: number }) {
  return <header className="project-bar"><div className="project-identity"><strong className="project-bar-title">{fileName.replace(/\.pdf$/i, "") || "Audiobook Studio"}</strong><span className="saved-badge"><StudioIcon name="save" size={14} />{saved ? "Salvo neste dispositivo" : "Processamento local"}</span></div><div className="project-statuses">{pageCount > 0 && <span><StudioIcon name="document" /><span>Documento importado<small>{pageCount} páginas</small></span></span>}{(audioBusy || chapterCount > 0) && <span><StudioIcon name={audioBusy ? "audio" : "check"} /><span>{audioBusy ? "Áudio em geração" : "Áudio disponível"}<small>{chapterCount ? `${chapterCount} capítulos` : "Preparando narração"}</small></span></span>}</div></header>;
}
export function AppShell({ children, fileName, pageCount, saved, hasDocument, audioBusy = false, chapterCount = 0 }: { children: ReactNode; fileName: string; pageCount: number; saved: boolean; hasDocument: boolean; audioBusy?: boolean; chapterCount?: number }) {
  ...
  return <div className={`app-frame${active === "review" && hasDocument ? " review-mode" : ""}`}>
    ...
    <div className="workspace-frame"><ProjectHeader fileName={fileName} pageCount={pageCount} saved={saved} audioBusy={audioBusy} chapterCount={chapterCount} /><main id="main-content" className={`shell${hasDocument ? " has-document" : ""}`}>{children}</main></div>
