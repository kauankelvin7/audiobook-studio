...
export function PageNavigator({ document, pageNumber, onPageChange }: Props) {
  ...
  return <aside className="page-rail" aria-label="Páginas do documento"><div className="result-heading"><h2 id="result-title">Páginas do documento</h2><span className="status-badge warning">{pending ? `${pending} sem texto` : `${document.pages.length} páginas`}</span></div>
...
export function DocumentViewer({ document, pageNumber, onPageChange, selectedRegion, onRegionSelect, sourcePdf }: Props) {
  ...
  return <div className="document-canvas" ref={viewer}><div className="document-toolbar"><div className="page-arrows"><button className="icon-button" onClick={() => onPageChange(Math.max(1,pageNumber-1))} disabled={pageNumber <= 1} aria-label="Página anterior">‹</button><button className="icon-button" onClick={() => onPageChange(Math.min(document.pages.length,pageNumber+1))} disabled={pageNumber >= document.pages.length} aria-label="Próxima página">›</button></div><span className="page-counter">Página {pageNumber} de {document.pages.length}</span>
    ...
    <div className="viewer-tools"><button className="icon-button" aria-label="Diminuir zoom" onClick={() => setZoom(Math.max(70,zoom-10))} disabled={zoom <= 70}>−</button><output aria-label="Zoom do documento">{zoom}%</output><button className="icon-button" aria-label="Aumentar zoom" onClick={() => setZoom(Math.min(200,zoom+10))} disabled={zoom >= 200}>+</button><button className="icon-button" aria-label="Expandir documento" onClick={() => { if (globalThis.document.fullscreenElement) void globalThis.document.exitFullscreen(); else void viewer.current?.requestFullscreen().catch(() => {}); }}><StudioIcon name="fullscreen" size={17} /></button><label className="review-toggle"><StudioIcon name="book" size={17} /><span>Modo de revisão</span><input type="checkbox" checked={review} onChange={event => setReview(event.target.checked)} /></label></div></div>
